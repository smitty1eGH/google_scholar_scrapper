...
#    information about all results. These are stored in a Pandas
#    DataFrame and then written to a csv.
#
...
hdr = {
    "User-agent": "Mozilla/5.0 (Windows; U; Windows NT 5.2; de; rv:1.9.1.5) Gecko/20091102 Firefox/3.5.5"
}
...
try:
    ...
    from urllib import Request, build_opener, HTTPCookieProcessor
...
while end_of_pages == False:
    ...
    try:
        ...
        print(
            "Error occurred trying to read page #%s: %s"
            % ((str(page_num), current_url))
        )
        end_of_pages = True
        break
    ...
    if len(page_results) == 0:
        ...
        print(
            "Note: This may mean that you have been temporarily blocked from Google Scholar."
        )
        print(
            "Try logging onto Google Scholar in Chrome, Firefox, or IE and completing the user authentication."
        )
        print("##############################################")
        break
    ...
    for result_soup in page_results:
        ...
        if authors_journal_year_text != "":
            ...
            if result_journal_year != "":
                if (
                    re.search("[1-2][0-9]{3}", result_journal_year[-4:]) is not None
                ) and (result_journal_year.count(", ") > 0):
    ...
    if (end_of_pages == False) and (records_extracted >= max_records):
        print(
            "We have reached the maximum number of records (%s). Exiting the page."
            % str(max_records)
        )
        end_of_pages = True
...
# full_df = full_df.iloc[:, col_list]
full_df.to_csv(join(workdir, outfile), index=False)

print(
    "Finished! Scraped %s pages and extracted %s records in total."
    % (str(page_num), str(records_extracted))
)
