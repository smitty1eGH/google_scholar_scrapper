    ...
    for result_soup in page_results:
        ...
        if authors_journal_year_text != '':
            ...
            if result_journal_year != '':
                if (re.search("[1-2][0-9]{3}", result_journal_year[-4:]) is not None) and (
                        result_journal_year.count(', ') > 0):
...
col_list = ['authors', 'title', 'description', 'year', 'journal', 'journal_website', 'link_1', 'link_2', 'link_3',
            'link_4']
